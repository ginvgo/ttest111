...
export async function onRequestPost(context) {
  ...
  try {
    ...
    `).run();
    // Ensure js_libraries exists for injection
  ...
  // === Fetch Dynamic Libraries ===
  let dynamicLibs = {};
  try {
      const { results } = await env.DB.prepare("SELECT * FROM js_libraries").all();
      if (results) {
          results.forEach(lib => {
              dynamicLibs[lib.name] = `<script src="${lib.url}"></script>`;
          });
      }
  } catch(e) {}

  const allLibs = { ...PRESET_LIBS, ...dynamicLibs };
  ...
  if (contentType.includes('application/json')) {
    ...
    projectName = formData.get('projectName') || folderName; 
  ...
  if (filesToUpload.length > 0) {
      for (const fileObj of filesToUpload) {
            ...
            if (fileObj.name.endsWith('.html')) {
                ...
                // INJECTION
                textContent = injectScripts(textContent, injectedLibs, allLibs);
  ...
  if (!isCodeEditMode) {
      ...
      // Code Edit Mode: Update injected_libs and updated_at
      await env.DB.prepare(`
        UPDATE projects SET injected_libs = ?, updated_at = datetime('now') WHERE folder_name = ?
      `).bind(injectedLibs, folderName).run();
  }

  return new Response(JSON.stringify({ success: true }));
}

function injectScripts(htmlContent, libsJson, allLibs) {
    ...
    // 1. Presets & Dynamic Libs
    if(config.presets) {
        config.presets.forEach(libName => {
            if(allLibs[libName]) {
                injection += allLibs[libName] + '\n';
            } else if (libName.startsWith('http') || libName.startsWith('//')) {
                 // Direct URL
                 injection += `<script src="${libName}"></script>\n`;
            }
        });
    }
